...
def test_windows_workspace_labeling_mutates_only_the_exact_empty_root(
    ...
    original = module._set_windows_mandatory_integrity_label

    def record(path: Path, *, directory: bool, integrity_rid: int) -> None:
        calls.append((path, directory, integrity_rid))
        original(path, directory=directory, integrity_rid=integrity_rid)

    monkeypatch.setattr(module, "_set_windows_mandatory_integrity_label", record)
    monkeypatch.setattr(
        module,
    ...
    assert calls == [(workspace.resolve(), True, 0)]
...
@pytest.mark.parametrize(
    ("is_restricted", "restricted_sid_hashes"),
    [
        (True, ()),
        (False, ("a" * 64,)),
    ],
)
def test_windows_startup_token_refuses_inherited_restrictions_before_native_derivation(
    monkeypatch: pytest.MonkeyPatch,
    is_restricted: bool,
    ...
    with pytest.raises(WriteConfinementUnavailable, match="already restricted caller"):
...
for name, target in (("medium", medium), ("low", low)):
    try:
        ...
        denied.append(name)
print(json.dumps({"denied": denied, "evidence": evidence.to_dict()}, sort_keys=True))
"""
