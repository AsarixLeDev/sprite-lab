...
def _resume_manifest(model: RectifiedFlowUNet) -> dict:
    ...
    conditioning = {"version": "spritelab_conditioning_v2", "mode": "test"}
    conditioning["hash"] = stable_hash(conditioning)
    optimizer = {"name": "adamw", "schedule": "none", "warmup_steps": 0}
    loss = {"name": "masked_velocity_mse"}
    seeds = {"training": 7}
    ema = {"enabled": True, "decay": 0.99}
    sampler = {"name": "stateful_permutation_v1"}
    manifest = {
        "manifest_version": EXPERIMENT_MANIFEST_VERSION,
        "dataset_manifest": str(source),
        "dataset_manifest_hash": file_sha256(source),
        "split_manifest": str(source),
        "split_manifest_hash": file_sha256(source),
        ...
        "schedule_identity_hash": stable_hash({"schedule": "none", "warmup_steps": 0}),
        "micro_batch_size": 1,
        "global_batch_size": 1,
        ...
        "sampler_policy_hash": stable_hash(sampler),
        "determinism_policy": "strict",
        "determinism_policy_hash": stable_hash("strict"),
        "evaluation_cadence": 1,
        "checkpoint_cadence": 1,
        "max_optimizer_steps": 1,
        "experiment_configuration_hash": stable_hash({"resolved": "test"}),
...
def test_unsafe_resume_records_every_bypassed_mismatch() -> None:
    ...
    assert set(mismatches).issubset(set(record["mismatches"]))
