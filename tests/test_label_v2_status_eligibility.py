...
def _make_status_run(tmp_path: Path) -> Path:
    ...
    records = [json.loads(line) for line in (run / "imported.jsonl").read_text(encoding="utf-8").splitlines() if line.strip()]
...
def test_label_v2_include_status_filters_when_requested(tmp_path: Path) -> None:
    ...
    rows = [json.loads(line) for line in (run / "accepted_only.jsonl").read_text(encoding="utf-8").splitlines() if line.strip()]
    assert len(rows) == 1
...
def test_label_v2_no_vlm_does_not_reuse_existing_qwen_suggestions(tmp_path: Path) -> None:
    run = _make_status_run(tmp_path)
    rows = [json.loads(line) for line in (run / "imported.jsonl").read_text(encoding="utf-8").splitlines() if line.strip()]
    ...
    predictions = [json.loads(line) for line in (run / "novlm.jsonl").read_text(encoding="utf-8").splitlines() if line.strip()]
