...
def test_resume_compatibility_and_rejection(tmp_path: Path) -> None:
    ...
    assert validate_resume_compatibility(changed, saved, unsafe=True) == ["conditioning_schema_hash"]
...
def test_smoke_checkpoint_resume_evaluation_export_and_paired_seed(tmp_path: Path) -> None:
    ...
    resumed = run_challenger_training(ChallengerTrainConfig(**base, max_steps=2, resume_from=checkpoint))
    assert first["steps_completed"] == 1
    assert resumed["steps_completed"] == 2
    assert resumed["resume_compatibility_mismatches"] == []
    assert resumed["ema_val_loss"] is not None
