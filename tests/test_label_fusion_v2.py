...
def _filename(name: str):
    return suggest_from_filename_v2(
        {"sprite_id": name, "relative_path": f"{name}.png", "source_id": "oga_cc0_food_ocal"}
    )
...
def test_agreement_weak_filename_and_degenerate_rules() -> None:
    ...
    vlm_apple = LabelSuggestion(
        category="item_icon", object_name="apple", tags=("fruit",), confidence=0.85, source="vlm_descriptor"
    )
    ...
    vlm = LabelSuggestion(
        category="item_icon", object_name="key", tags=("key",), confidence=0.85, source="vlm_descriptor"
    )
    weak_profile = suggest_from_filename_v2(
        {"sprite_id": "tile_001", "relative_path": "tile_001.png", "source_id": "unknown"}
    ).profile
    ...
    degenerate = LabelSuggestion(
        category="unknown",
        object_name="unknown",
        confidence=0.95,
        warnings=("degenerate_response",),
        source="vlm_descriptor",
    )
...
def test_profile_category_correction_and_conflict_prefill() -> None:
    tool = suggest_from_filename_v2(
        {"sprite_id": "compass", "relative_path": "compass.png", "source_id": "oga_cc0_tool_ocal"}
    )
    vlm = LabelSuggestion(
        category="item_icon", object_name="compass", tags=("navigation",), confidence=0.85, source="vlm_descriptor"
    )
    fused = fuse_label_v2(tool.suggestion, vlm, None, profile=tool.profile)
    assert fused.safe_prefill.category == "tool"

    gem = suggest_from_filename_v2(
        {"sprite_id": "ruby", "relative_path": "ruby.png", "source_id": "oga_cc0_gem_7soul1"}
    )
