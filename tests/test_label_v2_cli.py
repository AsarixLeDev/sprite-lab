...
def test_fuse_prefill_v2_writes_safe_suggestions(tmp_path, capsys) -> None:
    ...
    (run / "qwen_suggestions.jsonl").write_text(json.dumps({"sprite_id": imported["sprite_id"], "category": "material", "object_name": "gold_bar", "tags": ["gold"], "confidence": 0.85}) + "\n", encoding="utf-8")
...
def test_label_v2_report_and_eval_write_outputs(tmp_path, capsys) -> None:
    ...
    golden.write_text(json.dumps({"sprite_id": "apple", "category": "item_icon", "object_name": "apple", "tags": ["fruit"]}) + "\n", encoding="utf-8")
    out = tmp_path / "eval.json"
    errors_out = tmp_path / "errors.jsonl"
    main(["prefill-eval-v2", "--golden", str(golden), "--runs", str(run), "--out", str(out), "--errors-out", str(errors_out)])
    assert out.exists()
...
def test_golden_lint_cli_writes_fix_suggestions(tmp_path, capsys) -> None:
    golden = tmp_path / "golden.jsonl"
    golden.write_text(
        json.dumps({"sprite_id": "a", "category": "effect_icon", "object_name": "ice_cream_sandwich", "tags": ["ice_cream_sandwich"]}) + "\n",
        encoding="utf-8",
    )
