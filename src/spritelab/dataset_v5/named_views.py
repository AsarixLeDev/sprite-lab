...
APPROVED_CONTRACT_SHA256 = {
    "dataset_v5_record.schema.json": "e8c98776e44eb40f928c9f98f5b2cf8f9a7c479adbe6b6a3ad8bc98ee2176bc5",
...
def validate_contract(
    ...
    if not canonical_provenance_fields <= set(record_schema.get("required", [])):
        errors.append("record schema omits canonical lineage/provenance bindings")
...
def _load_policy(path: Path, view_name: str) -> dict[str, Any]:
    ...
    if not isinstance(synthetic_fixture, bool):
        raise _error("synthetic_fixture must be an explicit boolean", 23, "unsupported_schema")
    if synthetic_fixture is not True and "code_identity" in policy:
        raise _error(
    ...
    if synthetic_fixture and status == "candidate":
        raise _error(
            "candidate builds cannot claim synthetic fixture status",
            23,
            "unsupported_schema",
        )
    if synthetic_fixture and policy.get("production_freeze_authorized") is True:
        raise _error(
            "production builds cannot claim synthetic fixture status",
            23,
            "unsupported_schema",
        )
    metadata_allowlist = policy.get("source_blob_metadata_allowlist", [])
    if (
        not isinstance(metadata_allowlist, list)
        or not all(isinstance(value, str) and value for value in metadata_allowlist)
        or metadata_allowlist != sorted(set(metadata_allowlist))
        or any(
            "\\" in value
            or PurePosixPath(value).is_absolute()
            or ".." in PurePosixPath(value).parts
            or PurePosixPath(value).suffix == ".rgba"
            for value in metadata_allowlist
        )
    ):
        raise _error(
            "source_blob_metadata_allowlist must be a sorted-unique list of confined POSIX paths",
            23,
            "unsupported_schema",
        )
...
def _validate_license_provenance(row: Mapping[str, Any], policy: Mapping[str, Any]) -> None:
    ...
    for key in ("creator_lineage", "distribution_platform"):
        if not isinstance(row.get(key), str) or not row.get(key):
            raise _error(f"record provenance requires {key}", 26, "provenance_failure")
    for key in ("acquisition_run_identity",):
        if not isinstance(row.get(key), str) or not row.get(key):
            raise _error(f"record provenance requires {key}", 26, "provenance_failure")
...
def _canonical_provenance_projection(record: Mapping[str, Any]) -> dict[str, Any]:
    return {key: copy.deepcopy(record.get(key)) for key in PROVENANCE_RECORD_FIELDS}


def _validate_identity_uniqueness(records: Sequence[Mapping[str, Any]]) -> None:
    collisions: list[dict[str, Any]] = []
    for field in ("record_id", "sprite_id", "source_record_id"):
        record_ids_by_value: dict[str, list[str]] = defaultdict(list)
        for record in records:
            record_ids_by_value[str(record.get(field) or "")].append(str(record.get("record_id") or ""))
        for value, record_ids in sorted(record_ids_by_value.items()):
            if not value or len(record_ids) > 1:
                collisions.append({"field": field, "value": value, "record_ids": sorted(record_ids)})
    if collisions:
        summary = "; ".join(
            f"{item['field']}={item['value']!r}: {', '.join(item['record_ids'])}" for item in collisions
        )
        raise _error(
            f"canonical identity collision(s): {summary}",
            23,
            "unsupported_schema",
...
def _adapt_source_row(
    ...
    *,
    ...
    logical_blob_path = PurePosixPath(blob_value)
    if (
        blob_source.is_absolute()
        or "\\" in blob_value
        or ".." in logical_blob_path.parts
        or len(logical_blob_path.parts) != 2
        or logical_blob_path.parts[0] != "blobs"
        or logical_blob_path.suffix != ".rgba"
    ):
        raise _error(
            "source blob paths must be confined content-addressed blobs/<hash>.rgba POSIX paths",
            23,
            "unsupported_external_path",
        )
    ...
    if logical_blob_path.name != f"{declared_rgba_hash}.rgba":
        raise _error("source blob filename disagrees with canonical RGBA identity", 23, "unsupported_schema")
    ...
    provenance_source_identity = _require_string(provenance, "source_url")
    record: dict[str, Any] = {
        ...
        "distribution_platform": _require_string(source, "distribution_platform"),
        "creator_lineage": _require_string(source, "creator_lineage"),
        "pack_identity": _require_string(source, "source_pack"),
        "artist_publisher_identity": _require_string(source, "sub_artist"),
        "acquisition_run_identity": _require_string(source, "acquisition_run_identity"),
        "license_identity": _require_string(source, "license"),
        "provenance_status": _require_string(source, "provenance_status"),
        "provenance_source_identity": provenance_source_identity,
        "provenance_artifact_bindings": [
            {
                "artifact_identity": manifest_identity,
                "role": "source_manifest",
                "sha256": manifest_sha256,
            },
            {
                "artifact_identity": f"{manifest_identity}::{logical_blob_path.as_posix()}",
                "role": "source_blob",
                "sha256": actual_blob_hash,
            },
        ],
    ...
    license_row = _canonical_provenance_projection(record)
...
def _bind_exact_blob_stores(
    source_bindings: Sequence[Mapping[str, Any]],
    policy: Mapping[str, Any],
) -> list[dict[str, Any]]:
    declared_by_root: dict[Path, dict[str, dict[str, str]]] = defaultdict(dict)
    for source in source_bindings:
        for blob in source.get("blobs", []):
            blob_path = _resolve_locator(blob.get("locator"))
            root = blob_path.parent.resolve()
            name = blob_path.name
            declared = declared_by_root[root]
            existing = declared.get(name)
            identity = {
                "sha256": str(blob.get("sha256")),
                "exported_rgba_hash": str(blob.get("exported_rgba_hash")),
            }
            if existing is not None and existing != identity:
                raise _error(
                    f"duplicate blob identity has contradictory bindings: {blob_path}",
                    22,
                    "changed_source_hash",
                )
            declared[name] = identity
    allowlist = set(policy.get("source_blob_metadata_allowlist", []))
    stores: list[dict[str, Any]] = []
    for root, declared in sorted(declared_by_root.items(), key=lambda item: item[0].as_posix()):
        if not root.is_dir():
            raise _error(f"source blob store is missing: {root}", 21, "missing_source_artifact")
        ...
        case_collisions = [sorted(names) for names in case_groups.values() if len(names) > 1]
        declared_names = set(declared)
        actual_blob_names = set(actual_names) - allowlist
        missing = sorted(declared_names - actual_blob_names)
        extra = sorted(actual_blob_names - declared_names)
        hash_names: dict[str, list[str]] = defaultdict(list)
        for path, name in zip(actual_paths, actual_names, strict=True):
            if name in allowlist:
                continue
            hash_names[_file_hash(path)].append(name)
        duplicate_identities = [sorted(names) for names in hash_names.values() if len(names) > 1]
        if missing or extra or case_collisions or duplicate_identities:
            raise _error(
                f"source blob-store inventory mismatch: {root}",
                22 if not missing else 21,
                "changed_source_hash" if not missing else "missing_source_artifact",
                details={
                    "missing": missing,
                    "extra_or_unexpected": extra,
                    "case_collisions": case_collisions,
                    "duplicate_blob_identities": duplicate_identities,
                },
            )
        stores.append(
            {
                "locator": _stored_locator(root, synthetic_fixture=policy.get("synthetic_fixture") is True),
                "declared_blob_sha256": {name: values["sha256"] for name, values in sorted(declared.items())},
                "metadata_allowlist": sorted(allowlist),
            }
        )
    return stores
...
def _load_sources(
    ...
    blob_stores = _bind_exact_blob_stores(source_bindings, policy)
    source_binding = {
...
def _relation_values(record: Mapping[str, Any]) -> dict[str, list[str]]:
    values: dict[str, list[str]] = {
        ...
        "creator_lineages": [str(record["creator_lineage"])],
    }
    optional = {
...
def _validate_record(
    ...
    *,
    ...
    if record.get("provenance_status") != "verified":
        raise _error("record provenance is not verified", 26, "provenance_failure")
    if record.get("pack_identity") != record.get("source_pack"):
        raise _error("canonical pack identity contradicts source_pack", 26, "provenance_failure")
    if record.get("artist_publisher_identity") != record.get("sub_artist"):
        raise _error("canonical artist/publisher identity contradicts sub_artist", 26, "provenance_failure")
    if record.get("license_identity") != record.get("license"):
        raise _error("canonical license identity contradicts license", 27, "license_failure")
    provenance = record.get("provenance")
    if not isinstance(provenance, Mapping):
        raise _error("canonical provenance object is missing", 26, "provenance_failure")
    for key in ("semantic_origin", "source_url", "attribution"):
        _require_string(provenance, key)
    if provenance.get("license_confirmed") is not True:
        raise _error("canonical provenance license is not confirmed", 27, "license_failure")
    if record.get("provenance_source_identity") != provenance.get("source_url"):
        raise _error("canonical provenance source identity is contradictory", 26, "provenance_failure")
    bindings = record.get("provenance_artifact_bindings")
    if not isinstance(bindings, list) or len(bindings) != 2:
        raise _error("canonical provenance artifact bindings are incomplete", 26, "provenance_failure")
    roles: dict[str, Mapping[str, Any]] = {}
    for binding in bindings:
        if not isinstance(binding, Mapping) or set(binding) != {"artifact_identity", "role", "sha256"}:
            raise _error("canonical provenance artifact binding is malformed", 26, "provenance_failure")
        role = _require_string(binding, "role")
        _require_string(binding, "artifact_identity")
        _require_hash(binding, "sha256")
        if role in roles:
            raise _error("duplicate canonical provenance artifact role", 26, "provenance_failure")
        roles[role] = binding
    if set(roles) != {"source_manifest", "source_blob"}:
        raise _error("canonical provenance artifact roles are incomplete", 26, "provenance_failure")
    if roles["source_manifest"].get("artifact_identity") != record.get("source_artifact"):
        raise _error("source-manifest provenance identity is contradictory", 26, "provenance_failure")
    if roles["source_blob"].get("sha256") != record.get("blob_sha256"):
        raise _error("source-blob provenance hash is contradictory", 26, "provenance_failure")
...
def _verify_source_bindings(binding: Mapping[str, Any], errors: list[str]) -> None:
    ...
    blob_stores = binding.get("blob_stores")
    if not isinstance(blob_stores, list):
        errors.append("source binding has no exact blob-store inventory")
    else:
        for store in blob_stores:
            if not isinstance(store, Mapping):
                errors.append("source blob-store binding row is malformed")
                continue
            root = _resolve_locator(store.get("locator"))
            declared = store.get("declared_blob_sha256")
            allowlist = store.get("metadata_allowlist")
            if not root.is_dir() or not isinstance(declared, Mapping) or not isinstance(allowlist, list):
                errors.append(f"source blob-store binding is incomplete: {root}")
                continue
            paths = sorted(path for path in root.rglob("*") if path.is_file())
            names = [path.relative_to(root).as_posix() for path in paths]
            actual_blob_names = set(names) - {str(value) for value in allowlist}
            if actual_blob_names != {str(value) for value in declared}:
                errors.append(f"source blob-store exact inventory mismatch: {root}")
            casefolded = [name.casefold() for name in names]
            if len(casefolded) != len(set(casefolded)):
                errors.append(f"source blob-store case collision: {root}")
            ...
            for path, name in zip(paths, names, strict=True):
                if name not in allowlist:
                    ...
                    if declared.get(name) != digest:
                        errors.append(f"source blob-store hash mismatch: {name}")
            if any(len(blob_names) > 1 for blob_names in hashes.values()):
                errors.append(f"source blob-store duplicate blob identity: {root}")
...
def _replay_view_from_bound_sources(
    ...
    if replay_binding.get("blob_stores") != binding.get("blob_stores"):
        raise _error("source replay blob-store inventory differs from the view binding", 22, "changed_source_hash")
...
def _verify_view_impl(contract_root: Path, view_root: Path) -> dict[str, Any]:
    ...
    try:
        _validate_identity_uniqueness([*records, *excluded])
    except DatasetV5ViewError as exc:
    ...
    for record_id, record in record_by_id.items():
        ...
        if row is None:
            continue
        expected_provenance = _canonical_provenance_projection(record)
        if row != expected_provenance:
            differing = sorted(
                key for key in set(row) | set(expected_provenance) if row.get(key) != expected_provenance.get(key)
            )
            errors.append(f"license/provenance manifest mismatch for {record_id}: {', '.join(differing)}")
    try:
