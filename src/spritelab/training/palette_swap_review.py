...
def review_palette_swap(config: PaletteSwapReviewConfig) -> PaletteSwapReviewResult:
    ...
    limit = len(records) if int(config.max_samples) <= 0 else int(config.max_samples)
    selected = records[:limit]

    evaluations = list(_evaluate_samples(Path(config.dataset_dir), selected, swap_config))
...
def _evaluate_samples(
    ...
    for record in records:
        ...
        if npz_row < 0 or npz_row >= int(np.asarray(arrays["alpha"]).shape[0]):
            continue
        yield _evaluate_one(record, arrays, npz_row, swap_config)
...
def _evaluate_one(
    ...
    row = {
        ...
        "material_conflict_drop_count": len(result.materials_dropped),
...
def aggregate_metrics(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    metrics = {
        ...
        "ineligible_count": total - len(eligible_rows),
        "applied_rate": (applied_count / float(total)) if total else 0.0,
        "applied_rate_eligible": (applied_count / float(len(eligible_rows))) if eligible_rows else 0.0,
        ...
        "material_conflict_drop_count": int(sum(int(row.get("material_conflict_drop_count") or 0) for row in applied_rows)),
...
def compute_red_flags(
    ...
    if conservative_rate_context and total > 0:
        if applied_rate < CONSERVATIVE_APPLIED_RATE_MIN:
            flags.append(
                _flag(
                    "applied_rate_too_low",
                    applied_rate,
                    CONSERVATIVE_APPLIED_RATE_MIN,
                    "Conservative palette-swap applied rate is below the 5-15% target band.",
                )
            )
        elif applied_rate > CONSERVATIVE_APPLIED_RATE_MAX:
            flags.append(
                _flag(
                    "applied_rate_too_high",
                    applied_rate,
                    CONSERVATIVE_APPLIED_RATE_MAX,
                    "Conservative palette-swap applied rate is above the 5-15% target band.",
                )
            )
    if applied <= 0:
        return flags
