...
def inspect_sampler_resume_state(state: Any, *, dataset_size: int, batch_size: int, num_workers: int) -> list[str]:
    """Return every exact-resume defect without mutating sampler or RNG state."""

    if not isinstance(state, Mapping):
        return ["sampler_state.missing"]
    mismatches: list[str] = []
    if state.get("schema_version") != SAMPLER_STATE_VERSION:
        mismatches.append("sampler_state.schema_version")
    expected = {
    ...
    for field, value in expected.items():
        ...
        if type(actual) is not int or actual != value:
            mismatches.append(f"sampler_state.{field}")
    permutation = state.get("permutation")
    if (
        not isinstance(permutation, list)
        or len(permutation) != dataset_size
        or any(type(value) is not int for value in permutation)
        or sorted(permutation) != list(range(dataset_size))
    ):
        mismatches.append("sampler_state.permutation")
    cursor = state.get("sample_cursor")
    if type(cursor) is not int or not 0 <= cursor <= dataset_size:
        mismatches.append("sampler_state.sample_cursor")
    for field in ("sampler_generator_state", "dataloader_generator_state"):
