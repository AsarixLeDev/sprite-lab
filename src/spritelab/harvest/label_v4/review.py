...
SCALAR_STRING_FIELDS = frozenset(
    {
        "canonical_object",
        "category",
        "domain",
...
OBJECT_FIELDS = frozenset({"shape"})


@dataclass(frozen=True)
class FieldReviewResult:
    """Authoritative result of one field-scoped correction submission."""

    saved_field: str
    parsed_value: Any
    human_outcome: str
    event_id: str
    record_completion_state: Mapping[str, Any]
    next_unresolved_field: str | None
    message: str
...
def _append_review_event_unlocked(target: Path, event: ReviewEvent) -> None:
...
def append_review_event(path: str | Path, event: ReviewEvent) -> None:
    ...
    with _thread_lock(target):
        with _exclusive_process_lock(target.with_suffix(target.suffix + ".lock")):
            _append_review_event_unlocked(target, event)


def append_review_event_once(path: str | Path, event: ReviewEvent, submission_token: str) -> tuple[ReviewEvent, bool]:
    """Atomically append unless this exact UI submission token is already present."""

    token = str(submission_token).strip()
    if not token:
    ...
    with _thread_lock(target):
        with _exclusive_process_lock(target.with_suffix(target.suffix + ".lock")):
            for existing in load_review_events(target, strict=True):
                if str(existing.metadata.get("submission_token") or "") == token:
                    same_target = (
                        existing.sprite_id == event.sprite_id
                        and existing.field_name == event.field_name
                        and existing.action == event.action
                        and existing.proposal_hash == event.proposal_hash
                        and existing.reviewed_value == event.reviewed_value
                    )
                    if not same_target:
                        raise ValueError("submission token was already used for a different field action")
...
def record_review_action(
    ...
    **kwargs: Any,
    ...
    submission_token = str(metadata.get("submission_token") or "")
    if submission_token:
        event, _duplicate = append_review_event_once(path, event, submission_token)
    else:
...
def field_value_shape(record: Mapping[str, Any], field_name: str) -> str:
    """Return the correction input shape for a field.

    Production Labeling-v4 fields use the explicit name sets above. Synthetic or
    future fields may declare ``value_shape`` or JSON-schema ``type`` on the field.
    """

    fields = _proposal_fields(record)
    if field_name not in fields:
        raise ValueError(f"unknown semantic field: {field_name}")
    field_value = fields[field_name]
    field_map = field_value if isinstance(field_value, Mapping) else {"value": field_value}
    declared = field_map.get("value_shape")
    schema = field_map.get("schema")
    if not declared and isinstance(schema, Mapping):
        declared = schema.get("type")
    aliases = {"array": "list", "object": "object", "integer": "integer", "number": "number", "boolean": "boolean"}
    if declared:
        normalized = aliases.get(str(declared), str(declared))
        if normalized in {"string", "list", "object", "integer", "number", "boolean"}:
            return normalized
        raise ValueError(f"{field_name}: unsupported declared value shape {declared!r}")
    if field_name in SCALAR_STRING_FIELDS:
        return "string"
...
def parse_field_correction(record: Mapping[str, Any], field_name: str, raw_user_value: Any) -> Any:
    """Parse one correction without treating valid zero or false values as missing."""

    shape = field_value_shape(record, field_name)
    if isinstance(raw_user_value, str):
        raw = raw_user_value.strip()
        if not raw:
            raise ValueError(f"{field_name} requires a nonempty {shape} value")
    else:
        raw = raw_user_value
        if raw is None:
            raise ValueError(f"{field_name} requires a nonempty {shape} value")
    ...
    if shape == "string":
        if not isinstance(raw, str):
            raise ValueError(f"{field_name} requires a text value")
        return raw

    parsed = raw
    if isinstance(raw, str):
        if shape == "list" and not raw.startswith("["):
            parsed = [item.strip() for item in raw.split(",") if item.strip()]
            if not parsed:
                raise ValueError(f"{field_name} requires a nonempty list")
        else:
            try:
                parsed = json.loads(raw)
            except json.JSONDecodeError as exc:
                raise ValueError(f"{field_name} requires valid JSON for a {shape}: {exc.msg}") from None
    ...
    if shape == "integer" and isinstance(parsed, bool):
        raise ValueError(f"{field_name} requires an integer")
    if shape == "number" and isinstance(parsed, bool):
        raise ValueError(f"{field_name} requires a number")
    if not isinstance(parsed, expected_types[shape]):
        raise ValueError(f"{field_name} requires a {shape} value")
    if shape in {"list", "object"} and not parsed:
        raise ValueError(f"{field_name} requires a nonempty {shape} value")
    return copy.deepcopy(dict(parsed) if shape == "object" else parsed)
...
def save_field_correction(
    ...
    """Validate, parse, append, reload, and summarize one correction action."""

    field_name = str(selected_field).strip()
    expected_field = str(review_context.get("displayed_field") or field_name)
    if field_name != expected_field:
        raise ValueError(f"stale field selection: displayed {expected_field}, submitted {field_name}")
    expected_sprite = str(review_context.get("sprite_id") or record.get("sprite_id") or "")
    expected_audit = str(review_context.get("audit_record_id") or record.get("audit_id") or "")
    if expected_sprite != str(record.get("sprite_id") or "") or expected_audit != str(record.get("audit_id") or ""):
        raise ValueError("stale record selection: correction target changed before save")
    expected_hash = str(review_context.get("proposal_hash") or immutable_proposal_digest(record))
    if expected_hash != immutable_proposal_digest(record):
        raise ValueError("stale proposal identity: reload the record before saving")

    parsed = parse_field_correction(record, field_name, raw_user_value)
    submission_token = str(review_context.get("submission_token") or uuid.uuid4().hex)
    mode = str(review_context.get("review_mode") or record.get("review_mode") or "assisted")
    provenance = record.get("model_provenance") if isinstance(record.get("model_provenance"), Mapping) else {}
    metadata = {
        **dict(review_context.get("metadata") or {}),
        "audit_id": expected_audit,
        "audit_record_id": expected_audit,
        "sprite_id": expected_sprite,
        "field_name": field_name,
    ...
    event = create_review_event(
        ...
        reviewer_id=str(review_context.get("reviewer_id") or "assisted_v4_semantic_gui"),
        session_id=expected_audit,
        metadata=metadata,
    )
    event, duplicate = append_review_event_once(corrections_path, event, submission_token)
    if immutable_proposal_digest(record) != before:
        raise RuntimeError("raw proposal mutated while recording review action")

    authoritative = load_review_events(corrections_path, strict=True)
    from spritelab.harvest.label_v4.two_pass import QualityResolution, semantic_completion

    quality = review_context.get("quality")
    if quality is None:
        quality = QualityResolution(expected_sprite, str(record.get("quality_state") or "quality_suitable"), None, 0, 0)
    completion = semantic_completion(record, authoritative, quality)
    unresolved = list(completion["unresolved_fields"])
    next_field = unresolved[0] if unresolved else None
    remaining = ", ".join(unresolved) if unresolved else "none"
    value_text = json.dumps(parsed, ensure_ascii=False)
    prefix = "Already saved" if duplicate else "Saved"
    message = f"{prefix} {field_name} = {value_text}\nEvent: {event.event_id}\nRemaining required fields: {remaining}"
    return FieldReviewResult(
        saved_field=field_name,
        parsed_value=parsed,
        human_outcome=event.human_outcome,
        event_id=event.event_id,
        record_completion_state=completion,
        next_unresolved_field=next_field,
        message=message,
