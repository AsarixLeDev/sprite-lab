"""CLI entry points for dataset-maker subcommands (migrated from __main__.py)."""
...
def run_dataset_maker_gui(args: list[str]) -> None:
    import argparse
...
def run_dataset_maker_import_export(args: list[str]) -> None:
    import argparse
...
def run_dataset_maker_prefill(args: list[str]) -> None:
    import argparse
    import json
...
def _prefill_png_paths(root: Path | None) -> list[Path]:
    if root is None:
        return []
    return sorted(
